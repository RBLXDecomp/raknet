...
#define BIG_PACKET_SIZE 20739936
...
int main(void)
{
	...
	if (peer1)
	{
		...
		peer1->Startup(1, 30, &socketDescriptor, 1);
	...
	if (peer2)
	{
		...
		peer2->Startup(1, 30, &socketDescriptor, 1);
		...
		if (peer1)
		{
			...
			while (packet)
			{
				if (packet->data[0]==ID_DOWNLOAD_PROGRESS)
				{
					...
					printf("Progress: msgID=%i Progress %i/%i Partsize=%i Ploss=%.1f Full=%i\n",
						(unsigned char) data[0],
						progress,
						total,
						partLength,
						100.0f * ( float ) rss->messagesTotalBitsResent / ( float ) rss->totalBitsSent,
