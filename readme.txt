...
Latest changes:
Removed WinAutoPatchProject
Increased telnet max text input
Fixed threading bug in ThreadPool - was causing the autopatcher to quit prematurely, so that the last few files would never get written
Add assertion to TableSerializer to make sure you specify a length for binary data
Ordered list now has a parameter to assert on duplicate insertions or not
Fix Windows Vista bug where LAN broadcasting wasn't working
Don't sort commands send to the replica manager anymore.  Process them in order so dependencies are handled correctly.
NetworkID manager now uses an O(1) hash by default, rather than an O(log2(n)) balanced binary tree.  However, this won't work for games with more than 65534 objects, or peer to peer.
Fixed a potential crash caused by a write to possibly deleted memory in the B Plus Tree
Don't endian swap the binary address part of SystemAddress in the BitStream class, for OSX compatibility
SetMTUSize no longer takes a specific system address
ThreadPool now has a callback to create per-thread data
Autopatcher client now returns ID_AUTOPATCHER_FINISHED when no files have been changed
PostgreRepository now has a function to get the timestamp on the server.  This makes it easier to roll out new builds that don't need to do a full scan
